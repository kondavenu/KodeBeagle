          ...
          var nextChunk = function nextChunk() {
            ...
            for (var i = 0; i < CHUNK_SIZE; i++) {
              ...
              if (type === 'tree') {
                ...
                item.a_attr = {
                  href: '/' + repo.username + '/' + repo.reponame + '/' + type + '/' + repo.branch + '/' + encodeURIComponent(_path)
                };
              } else if (type === 'commit') {
...
var GH_RESERVED_USER_NAMES = ['settings', 'orgs', 'organizations', 'site', 'blog', 'about', 'explore', 'styleguide', 'showcases', 'trending', 'stars', 'dashboard', 'notifications', 'search', 'developer', 'account', 'pulls', 'issues', 'features', 'contact', 'security', 'join', 'login', 'watching', 'new', 'integrations', 'gist'];
...
var GL_HEADER = '.navbar-gitlab';
var GL_SIDEBAR = '.sidebar-wrapper';
var GL_TITLE = 'h1.title';
var GL_CONTENT = '.container>.content';
var GL_MAIN_NAV = '.main-nav';
...
var GitLab = function (_Adapter) {
  ...
  _createClass(GitLab, [{
    ...
    key: 'updateLayout',
    value: function updateLayout(togglerVisible, sidebarVisible, sidebarWidth) {
      var isNewDesign = $('.navbar-gitlab.header-collapsed, .navbar-gitlab.header-expanded').length > 0;
      var glSidebarExpanded = $('.page-with-sidebar').hasClass('page-sidebar-expanded') && $(document).width() > 1199;

      if (isNewDesign) {
        var glSidebarWidth = glSidebarExpanded ? 230 : 62;
    ...
    value: function _get(path, opts, cb) {
      ...
      var project = $('#project_id').val() || repo.username + '%2f' + repo.reponame;
...
var TreeView = function () {
  ...
  _createClass(TreeView, [{
    ...
    value: function _showHeader(repo) {
      ...
      this.$view.find('.octotree_view_header').html('<div class="octotree_header_repo">' + '<a href="/' + repo.username + '">' + repo.username + '</a>' + ' / ' + '<a data-pjax href="/' + repo.username + '/' + repo.reponame + '">' + repo.reponame + '</a>' + '</div>' + '<div class="octotree_header_branch">' + repo.branch + '</div>').on('click', 'a[data-pjax]', function (event) {
        event.preventDefault();
